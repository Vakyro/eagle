...
export default function QueuePage({ params }: QueuePageProps) {
  ...
  const [service, setService] = useState<ServiceWithStats | null>(null)
  ...
  useEffect(() => {
    ...
    if (!user) {
      console.log('No hay usuario en queue page, redirigiendo al login')
      router.push("/login")
      return
    }
    ...
    console.log('Usuario válido en queue page:', user.email)
  ...
  const loadService = async () => {
    try {
      ...
      // Get service details
      const serviceData = await serviceService.getById(params.id)
          ...
          <div className="p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
              <div className="flex-1">
                ...
                <p className="text-gray-600 mt-1">{service.service_type}</p>
              </div>
              <div
                className={`self-start px-3 py-1 rounded-full text-sm font-medium ${
                  service.is_open ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                }`}
              >
            ...
            <div className="flex items-start gap-2 text-sm text-gray-600 mb-4">
              ...
              <span className="break-words">{service.establishment?.address || "Address not available"}</span>
            </div>

            <div className="text-sm text-gray-600">
              <strong>Hours:</strong> {service.operating_hours || "Hours not available"}
            </div>
          </div>
        </Card>
              ...
              <div className="bg-gray-50 rounded-lg p-3 sm:p-4 mb-4">
                <div className="flex justify-between items-center text-sm mb-2">
                  ...
                  <span className="font-medium text-[#050315]">
                    {service.queueCount}/{service.max_capacity}
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className="bg-blue-600 h-2 rounded-full transition-all duration-300"
                    style={{ width: `${(service.queueCount / service.max_capacity) * 100}%` }}
                  />
                </div>
              </div>

              <JoinQueueButton serviceId={service.id} serviceName={service.name} serviceType={service.service_type} />
            </div>
          </Card>
        )}
        ...
        {!service.is_open && (
          <Card className="bg-red-50 border border-red-200 shadow-sm rounded-xl overflow-hidden">
            <div className="p-4 sm:p-6 text-center">
              ...
              <p className="text-sm text-red-600">Hours: {service.operating_hours || "Hours not available"}</p>
            </div>
          </Card>
        )}
      </div>
